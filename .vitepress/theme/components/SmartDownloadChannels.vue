...
function normalizePlatform(raw: string) {
  ...
  if (/(iphone|ipad|ipod|ios)/.test(value)) return "ios" as const;
  if (/android/.test(value)) return "android" as const;
  if (/(mac|darwin|os x)/.test(value)) return "macos" as const;
  if (/(win|windows)/.test(value)) return "windows" as const;
  if (/linux|x11/.test(value)) return "linux" as const;
...
function normalizeArch(raw: string) {
  ...
  if (/(arm|armv7)/.test(value)) return "arm" as const;
...
async function detectEnvironment() {
  ...
  if (detectedPlatform.value === "macos") {
    const macArchitectureHints = [nav.userAgentData?.architecture].filter(
      Boolean,
    ) as string[];
    ...
    detectedArch.value =
      detectedMacArch === "unknown" ? "arm64" : detectedMacArch;
    return;
  }
...
const recommendation = computed<DownloadRecommendation>(() => {
  ...
  if (detectedPlatform.value === "windows" && detectedArch.value === "x64") {
    return {
      ...
      docLink: {
        label: "版本区别",
        url: "/Started/ConnectionAndUpdate#版本区别",
      },
    };
  }
  ...
  if (
    ...
    (detectedArch.value === "arm64" || detectedArch.value === "arm")
  ) {
    return {
...
function getPreferredScrollTarget() {
  ...
  while (current) {
    if (/^H[1-6]$/.test(current.tagName)) return current as HTMLElement;
...
async function revealChannels() {
  if (detailsRef.value && !detailsRef.value.open) detailsRef.value.open = true;
  ...
  <section ref="rootRef" class="smart-download" aria-label="智能下载渠道推荐">
    <div class="smart-download__hero">
      ...
      <p v-if="recommendation.versionNote" class="smart-download__version-note">
        ...
        <a v-if="recommendation.docLink" :href="recommendation.docLink.url">{{
          recommendation.docLink.label
        }}</a>
      </p>
    ...
    >
      ...
      <div id="download-channel-links" class="smart-download__grid">
        ...
        >
          ...
          <span v-if="channel.note" class="smart-download__link-note">{{
            channel.note
          }}</span>
        </a>
      </div>
